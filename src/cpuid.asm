...
; uint32_t _get_cpuid_leaf_asm(uint32_t leaf, uint8_t register_id)
global _get_cpuid_leaf_asm
_get_cpuid_leaf_asm:
    mov eax, [esp+0x4]
    cpuid
    cmp byte [esp+0x8], CPUID_EAX_ASM
    jne .not_eax
    jmp .r
.not_eax:
    cmp byte [esp+0x8], CPUID_EBX_ASM
    jne .not_ebx
    mov eax, ebx
    jmp .r
.not_ebx:
    cmp byte [esp+0x8], CPUID_ECX_ASM
    jne .not_ecx
    mov eax, ecx
    jmp .r
.not_ecx:
